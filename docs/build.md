...
- `CaseDashTools.exe format` embeds the strictfmt CLI implementation behind `format.cmd` and clang-format-like IDE integration. It reads stdin when no file input is present, accepts direct file arguments, recursive roots through `-r <path>` or `--recursive <path>` for `.c`, `.cc`, `.cpp`, `.cxx`, `.c++`, `.h`, `.hh`, `.hpp`, `.hxx`, `.h++`, `.ipp`, `.inl`, and `.tpp` files, and newline file lists through `--files <path>` or `--files=<path>`, supports `-i`, `--dry-run` or `-n`, `--style=file`, `--style=<path>`, `--style=file:<path>`, `--concurrency <n>`, and `--verbose`, locates formatter config and ignore files as described in `external\strictfmt\docs\format.md`, parses each source through the strictfmt tree-sitter C++ grammar, rejects sources whose parse contains errors or missing nodes, and reports completed file counts plus elapsed time. The standalone `strictfmt.exe` target is built from the same source under `external\strictfmt`.
