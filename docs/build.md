...
- `format.cmd` is the maintained entrypoint for repository formatting. It reuses an existing `build\CaseDashTools.exe`, builds `build\CaseDashTools.exe` and `build\strictfmt.exe` through `build.cmd /tools` only when the tools executable is missing, then runs plain `format` and `format fix` as `CaseDashTools.exe format [-i] -r .` so native timing and progress include recursive file discovery. `format.cmd changed` and `format.cmd fix staged --restage` use `tools\git_file_list.ps1` to prepare a newline-separated list with the formatter's default recursive suffix set from git first, merge strictfmt submodule changed or staged files with `external/strictfmt/` prefixes, then delegate that list to `CaseDashTools.exe format --files` so the native tool does not call git. Pass `--concurrency <n>` to override native formatter worker count. Successful check and fix summaries report completed file counts, LOC, and elapsed time.
- `CaseDashTools.exe format` embeds the strictfmt CLI implementation behind `format.cmd` and IDE integration. It reads stdin only when `--stdin` is passed, accepts direct file arguments, recursive roots through `-r <path>` or `--recursive <path>` for `.c`, `.cc`, `.cpp`, `.cxx`, `.c++`, `.h`, `.hh`, `.hpp`, `.hxx`, `.h++`, `.ipp`, `.inl`, and `.tpp` files, and newline file lists through `--files <path>`, supports `-i`, `--dry-run` or `-n`, `--style <path>`, `--concurrency <n>`, and `--verbose`, locates formatter config and ignore files as described in `external\strictfmt\docs\format.md`, parses each source through the strictfmt tree-sitter C++ grammar, rejects sources whose parse contains errors or missing nodes, and reports completed file counts, LOC, and elapsed time. Running the embedded formatter with no input prints help. The standalone `strictfmt.exe` target is built from the same source under `external\strictfmt`.
...
- `external\strictfmt\tools\regenerate_tree_sitter_grammar.py` is the explicit maintenance tool for regenerating vendored tree-sitter C++ grammar outputs after editing `external\strictfmt\vendor\tree-sitter\tree-sitter-cpp\grammar.js` or parser macro categories in `.cpp-format` or `external\strictfmt\tests\format\.cpp-format-userver`. It writes `macro_config.js`, runs the pinned tree-sitter CLI, and keeps generated parser files under `external\strictfmt\vendor\tree-sitter\tree-sitter-cpp\src\`. If no CLI path is provided, it downloads the pinned Windows CLI under `external\strictfmt\build\`.
- `test.cmd` is the maintained entrypoint for running the CTest suite against the built Release tree, including strictfmt's registered formatter tests and the CaseDash unit tests, then the lint-check regression fixture under `tools\tests\lint`. It prints verbose CTest output so GitHub runner logs show each test command and its stdout even when tests pass.
