...
- Release app builds compile with `/GL` and link with `/LTCG`, `/OPT:REF`, and non-incremental linking so whole-program optimization and reference elimination reduce the shipped executable. Tests and benchmarks keep the normal Release compile/link path for faster local validation.
