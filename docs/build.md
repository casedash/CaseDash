...
- `external\strictfmt\tools\regenerate_tree_sitter_grammar.py` is the explicit maintenance tool for regenerating vendored tree-sitter C++ grammar outputs after editing `external\strictfmt\vendor\tree-sitter\tree-sitter-cpp\grammar.js`. It runs the pinned tree-sitter CLI and keeps generated parser files under `external\strictfmt\vendor\tree-sitter\tree-sitter-cpp\src\`. Macro categories in `.cpp-format` are runtime scanner inputs and do not affect parser generation. If no CLI path is provided, the tool downloads the pinned Windows CLI under `external\strictfmt\build\`.
- `test.cmd` is the maintained entrypoint for running the CTest suite against the built Release tree, including strictfmt's registered formatter tests and the CaseDash unit tests, then the lint-check regression fixture under `tools\tests\lint`. CaseDash directs strictfmt's formatter tests to `build\CaseDashTools.exe format` rather than a standalone `strictfmt.exe`. It prints verbose CTest output so GitHub runner logs show each test command and its stdout even when tests pass.
