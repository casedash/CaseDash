...
- `format.cmd` is the maintained entrypoint for repository formatting. It builds `build\CaseDashTools.exe` through `build.cmd /tools` when the tool is missing, then runs plain `format` and `format fix` as `CaseDashTools.exe format [-i] -r .` so native timing and progress include recursive file discovery. `format.cmd changed` and `format.cmd fix staged --restage` use `tools\git_file_list.ps1` to prepare a newline-separated list with the formatter's default recursive suffix set from git first, then delegate that list to `CaseDashTools.exe format --files` so the native tool does not call git. Pass `--concurrency <n>` to override native formatter worker count. Successful check and fix summaries report completed file counts plus elapsed time.
- `CaseDashTools.exe format` is the tree-sitter formatter subcommand behind `format.cmd` and clang-format-like IDE integration. It reads stdin when no file input is present, accepts direct file arguments, recursive roots through `-r <path>` or `--recursive <path>` for `.c`, `.cc`, `.cpp`, `.cxx`, `.c++`, `.h`, `.hh`, `.hpp`, `.hxx`, `.h++`, `.ipp`, `.inl`, and `.tpp` files, and newline file lists through `--files <path>` or `--files=<path>`, supports `-i`, `--dry-run` or `-n`, `--style=file`, `--style=<path>`, `--style=file:<path>`, `--concurrency <n>`, and `--verbose`, locates formatter config and ignore files as described in `docs\format.md`, parses each source through the vendored tree-sitter C++ grammar compiled into `CaseDashTools`, rejects sources whose parse contains errors or missing nodes, applies the fixed formatter rules encoded in `src\tools\format.cpp` and internal `src\tools\impl\format_*` formatter modules, and reports completed file counts plus elapsed time.
