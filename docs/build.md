...
If `build.cmd` starts spending minutes at `Detecting compiler hash for triplet x64-windows`, delete `build\cmake` and rerun `build.cmd`. That symptom means the CMake build tree can be carrying stale Visual Studio toolset detection while vcpkg is probing the current compiler; the repo-root `vcpkg\` install tree and shared downloads cache are preserved.
