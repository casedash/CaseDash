...
- `external\strictfmt\docs\format.md` is the maintained source of truth for C++ formatting policy, formatter configuration, and parser support. `external\strictfmt\src\format\format_cli.cpp`, internal `external\strictfmt\src\format\impl\format_*` formatter modules, `.cpp-format`, `.cpp-format-ignore`, and the vendored tree-sitter runtime and grammar under `external\strictfmt\vendor\tree-sitter\` implement that policy. The golden formatter fixture under `external\strictfmt\tests\format\src` stays synchronized with formatter output produced through strictfmt's standalone build flow and, inside CaseDash validation, through the embedded `CaseDashTools.exe format` subcommand.
...
- Keep package-managed dependencies in `vcpkg.json` when practical. strictfmt-owned parser runtime and grammar sources that must be generated or locally adapted live under `external\strictfmt\vendor\`, outside maintained formatter and lint source discovery.
...
- Keep shipped app and test filesystem operations on `src/util/file_path.*` helpers instead of `std::filesystem`; paths are stored as UTF-8 and use Win32 A APIs by default. Repository-only tooling under `src/tools/` and the standalone formatter under `external/strictfmt/src/` may use `std::filesystem`. `WideForNativeApi` is reserved for wide-native interfaces such as WIC filename methods and Desktop Wallpaper COM. `lint.cmd` enforces this source-policy rule for maintained source and test files.
