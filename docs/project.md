...
- `external\strictfmt\docs\format.md` is the maintained source of truth for C++ formatting policy, formatter configuration, and parser support. `external\strictfmt\src\tools\format.cpp`, internal `external\strictfmt\src\tools\impl\format_*` formatter modules, `.cpp-format`, `.cpp-format-ignore`, and the vendored tree-sitter grammar under `external\strictfmt\vendor\tree-sitter\` implement that policy. The golden formatter fixture under `external\strictfmt\tests\format\src` stays synchronized with formatter output produced through `strictfmt` and the embedded `CaseDashTools.exe format` subcommand.
