...
- Keep committed resource payloads as the source of truth; CMake generates the compressed embedded config and localization resources under `build\cmake\generated\`, while `resources/CaseDash.rc` keeps the directly embedded app icon and panel-icon atlas explicit.
