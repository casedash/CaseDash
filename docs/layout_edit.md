...
- Metric leaves whose ids begin with `board.temp.` or `board.fan.` also expose a live `Binding` selector for the matching board-sensor mapping. Fallback-backed metric leaves, including `[metrics] gpu.fan` and `[metrics] gpu.temp`, expose the same selector for their shared board binding target.
