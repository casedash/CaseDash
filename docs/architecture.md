...
- `CMakeLists.txt` enables C4505 as an error for MSVC C++ targets and keeps Release app links non-incremental with `/OPT:REF` and `/LTCG`; the app target compiles Release objects with `/GL` so whole-program optimization can reduce the shipped executable.
