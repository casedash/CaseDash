...
- `src/tools/` contains native repository tools built into `build\CaseDashTools.exe`, including the maintained combined lint scanner behind `lint.cmd` and the embedded strictfmt subcommand behind `format.cmd`. `external/strictfmt/` contains the formatter source, formatter tests, standalone `strictfmt` target, vendored tree-sitter grammar sources, and [formatter configuration reference](../external/strictfmt/docs/format.md). `tools/` contains script entrypoints for profiling, generated assets, and compatibility helpers. `tools/lint_config.json` owns lint policy data.
