...
- Current largest sections: `.text$mn` about `949.7 KiB`, `.rdata` about `88.2 KiB`, `.rsrc$02` about `33.5 KiB`, `.pdata` about `20.8 KiB`, `.xdata` about `19.8 KiB`.
...
| Panel icon atlas | Keep panel icons in one fixed 64 x 64 vertical PNG atlas and draw cropped atlas slots through a target-local alpha mask instead of caching per-icon sources and scaled Direct2D bitmaps. | `1,176,576` to `1,171,456` bytes; `.text$mn` dropped from about `954.2 KiB` to `949.7 KiB` and `.rsrc$02` dropped from about `34.0 KiB` to `33.5 KiB`. |
...
- Keep the panel-icon atlas simple and fixed-slot. A literal per-draw tinted bitmap reached `1,170,944` bytes but regressed `edit-layout` paint to about `2.50 ms`; keep the target-local alpha-mask version unless a future icon-heavy benchmark proves another shape is better.
