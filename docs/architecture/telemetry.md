...
- Native vendor providers keep provider-specific setup and method translation inside the provider module, including contracts such as ASUS ATKACPI `DSTS` device calls, Lenovo Hardware Scan LdeApi calls, Lenovo's GameZone WMI fan fallback, and vendor SDK bridges, then publish normal board-provider samples to the collector. Slow direct provider fallbacks own their own refresh cadence and cached sample state so telemetry startup does not wait on vendor diagnostics scans.
