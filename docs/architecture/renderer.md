...
- Provide generic opaque layer bitmap rendering and bitmap composition primitives through `RenderBitmap`, `DrawToBitmap()`, `DrawBitmap()`, and `DrawBitmapRegion()`. `RenderBitmap` is resource-backed only; backend-specific Direct2D/WIC resources stay hidden behind `RenderBitmapResource`.
