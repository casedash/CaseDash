...
- Measure formatter golden throughput with `build\CaseDashBenchmarks.exe format-golden 20 2`; the benchmark formats `external\strictfmt\tests\format\src\format_test_input.cpp` in memory and reports total plus per-iteration time without file writes. It also reports formatter phase splits for parse, print, tokenization, break-model construction, DP solve, output emission, and source comparison.
