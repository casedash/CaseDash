...
- The animation benchmark keeps layout, telemetry, and layer painting out of the timed loop. It times the render-thread side of each animation frame: target-local bitmap composition, active transition sampling, timeline state lookup, and widget animation drawing.
...
- The direct `animation` benchmark builds one fake-metric, no-overlay dashboard frame with `28` snapshot animations and repeatedly presents the stored frame through the render-thread presenter path. Current direct reruns land at `animation_frame avg_ms=0.48` to `0.50`, isolating per-frame bitmap composition, active `WidgetAnimationTransition::Sample()` work, `DashboardAnimationTimeline` lookup, and widget animation drawing from telemetry, layout, and layer painting.
- The daemon-backed `animation` capture under `build\profile_benchmark_daemon\requests\2555_14684_26090\` used `2400` frames and reported `animation_frame avg_ms=0.50` with `active_chunk_frames=120`. The hotspot shape is dominated by Direct2D present work: `D2DRenderer::EndDirect2DDraw` is the largest app-inclusive frame leaf, followed by `DashboardRenderThread::DrawFrame`, `DashboardRenderThread::DrawAnimations`, and throughput chart animation drawing through `ThroughputChartAnimation::Draw` and `DrawGraphAnimated`.
