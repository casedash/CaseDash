...
- Measure the snapshot handoff benchmark with `build\CaseDashBenchmarks.exe snapshot-handoff 240 2`; this benchmark uses fake metrics, forces a new snapshot revision each iteration, rebuilds the no-overlay snapshot layer into pooled hardware-backed layer bitmaps, hands the complete frame to the render thread, waits for immediate non-vsynced presentation, and reports frame-build cost separately from render-thread handoff and synchronization cost.
...
- The layout, hover, theme, telemetry, and guide-sheet benchmark hosts force the dashboard renderer's immediate-present path. Direct benchmark runs for those paint paths stay single-threaded, but paint timings include snapshot and overlay bitmap construction plus the final bitmap-and-animation composition step that the live render thread performs in production. The `animation` benchmark builds one threaded-style layer frame, then drives a benchmark render worker around the stored frame so the timed loop includes request handoff without waiting for vsync. The `snapshot-handoff` benchmark keeps snapshot layer construction in the main-thread renderer and waits for a render-thread immediate presentation, so it exposes cross-thread handoff, layer-bitmap reuse, and synchronization costs without monitor cadence.
...
- Current repeatable `snapshot-handoff` result on the current tree:
  - `snapshot_loop per_iter_ms=2.51`
  - `presentation_frame_build avg_ms=1.71`
  - `presentation_frame_handoff avg_ms=0.81`
...
- The direct `snapshot-handoff` benchmark rebuilds a fake-metric, no-overlay snapshot frame every iteration, uses the dashboard layer-bitmap pool, and waits for immediate render-thread presentation. The first direct run reports `snapshot_loop per_iter_ms=2.51`, split into `presentation_frame_build avg_ms=1.71` and `presentation_frame_handoff avg_ms=0.81`; this is the comparison target for live `/trace-prefixes:profile` windows where `snapshot_layer_bitmap` dominates `paint_draw`.
