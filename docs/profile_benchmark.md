...
- Current repeatable `snapshot-handoff` result on the current tree:
  - `snapshot_loop per_iter_ms=10.57`
  - `presentation_frame_build avg_ms=10.57`
...
- The direct `snapshot-handoff` benchmark rebuilds a fake-metric, no-overlay snapshot frame every iteration while a visible benchmark HWND presents active animations on vsync, uses the dashboard layer-bitmap pool, and publishes through the live asynchronous render-thread mailbox. The cadence-aware run reports `snapshot_loop per_iter_ms=10.57`, split into `presentation_frame_build avg_ms=10.57` and `presentation_frame_publish avg_ms=0.00`, which matches the live `/trace-prefixes:profile` window where `snapshot_layer_bitmap` dominates `paint_draw` at about `10-11 ms`. The important benchmark ingredients are an active live-animation render thread, a visible HWND with a message pump, and starting the next telemetry-style build before the previous 500 ms animation fully expires.
