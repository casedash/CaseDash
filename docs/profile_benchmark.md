...
- Real `/trace` runs emit `profile:timing` summaries every 10 seconds for comparable runtime operation names such as `telemetry_update`, `paint_draw`, and `animation_frame`; use those summaries to check whether direct benchmark timings match the interactive app on the same machine.
