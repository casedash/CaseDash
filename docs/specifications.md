...
- Board telemetry selects the provider from the baseboard manufacturer, reads supported board temperatures and fan speeds through the active provider, uses the CaseDash service when a supported provider needs privileged hardware access, allows slow direct vendor fallbacks to refresh in the background while retaining the last successful values, and uses an unsupported-board provider with unavailable values when no supported board provider matches.
